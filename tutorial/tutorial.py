...
BULLET_HIT_SOUND = pygame.mixer.Sound(os.path.join("tutorial/Assets", "Grenade+1.mp3"))
BULLET_FIRE_SOUND = pygame.mixer.Sound(os.path.join("tutorial/Assets", "Gun+Silencer.mp3"))
...
def handle_yellow_movement(keys, yellow):
    ...
    if keys[pygame.K_s] and yellow.y + yellow.height + 16 < HEIGHT:  # DOWN
        yellow.y += VEL

    if keys[pygame.K_a] and yellow.x - VEL > 0:  # LEFT
        yellow.x -= VEL

    if keys[pygame.K_d] and yellow.x + yellow.width - 2 * VEL < WIDTH / 2:  # RIGHT
        yellow.x += VEL
...
def handle_red_movement(keys, red):
    ...
    if keys[pygame.K_DOWN] and red.y + red.height + 16 < HEIGHT:  # DOWN
        red.y += VEL

    if keys[pygame.K_LEFT] and red.x > WIDTH / 2 + 7:  # LEFT
        red.x -= VEL

    if keys[pygame.K_RIGHT] and red.x + red.width - 3 * VEL < WIDTH:  # RIGHT
        red.x += VEL


# Handle Movement, Collision & Out of Bounds
def handle_bullets(yellow_bullets, red_bullets, yellow, red):
    for bullet in yellow_bullets:
        ...
        elif bullet.x > 900 - bullet.width:
            yellow_bullets.remove(bullet)
